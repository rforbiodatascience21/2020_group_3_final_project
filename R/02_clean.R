...
my_data_clean <- my_data %>%
  mutate(Age = case_when(Age == "greater then 15" ~ "> 15",
         ...
         BMI = round(BMI,1),
         Dur_disease(days)(days) = str_extract(`Duration of disease`,"\\d+\\.?\\d*"),
...
my_data_clean <- my_data_clean %>%
  mutate(Dur_disease(days)(days) = as.numeric(Dur_disease(days)(days))) %>%
  mutate(Dur_disease(days)(days) = case_when(unit == "d" ~ Dur_disease(days)(days),
